...
public:
    ...
    player() : name(""), player_snake_score(0), player_wordle_score(0), player_hangman_score(0) {}

    player(const string& _name, int ss, int ws, int hs) : name(_name), player_snake_score(ss), player_wordle_score(ws), player_hangman_score(hs) {}
    ...
    {
        ofstream outFile("players_data.txt", ios::app); 


        if (outFile.is_open())
         {
           
            outFile << name << "," << player_snake_score << "," << player_wordle_score << "," << player_hangman_score << endl;
            outFile.close(); 
...
public:
    ...
    // Destructor 
    ~category() {
        delete[] name; // now Free memory for the name
        if (words != nullptr) {
            for (int i = 0; i < word_count; i++) {
                delete[] words[i]; // now Free memory for each word
            }
            delete[] words; // now Free the array of word pointers
        }
    }
};
...
public:
    ...
    void resetScore() {
     ...
     void saveHighScore() {
        ofstream hm_file("hangman_highscore.txt");
        if (hm_file.is_open()) {
            hm_file << highScore;
            hm_file.close();
        } 
    ...
    void loading_HighScore() {
        ifstream hm_file("hangman_highscore.txt");
        if (hm_file.is_open()) 
        {
            hm_file >> highScore;
            hm_file.close();
        } else {
            highScore = 0; //if file doesn't exist
        }
    }
    void updateHighScore() {
        if (score > highScore) {
            highScore = score; // Updating the high score if current score is greater
            saveHighScore();  
            //saving the updated
        }
    }
    ...
    void displayScore(sf::RenderWindow& window, sf::Font& font) {
        ...
        sf::Text highScoreText("High Score: " + to_string(highScore), font, 30);
        highScoreText.setFillColor(sf::Color::Black);
        highScoreText.setPosition(990.f, 100.f);
...
public:
    ...
    HangmanGame() : window(sf::VideoMode(800, 600), "Hangman Game") {
        ...
        endMessage.setPosition(400.f, 300.f); 
...
//splash is the image displayed when game is started
 void splash(sf::RenderWindow &window, unsigned int w, unsigned int h) 
 {
    ...
    if (!splashTexture.loadFromFile("hangmansplash.jpeg"))
     {
...
void selectCategory(const char* categoryName) {
    ...
    for (int i = 0; i < categoryCount; i++) {
        ...
        if (strcmp(current_category, categoryName) == 0) {
            ...
            //randomWord= "Sea Green";
           // randomWord="Blue Whale";
...
void drawDashes() {
    ...
    for (int i = 0; guessedWord[i] != '\0'; i++) {
        if (selectedWord[i] == ' ') {
            dashes += "  ";  // Adding double space for spaces
        } else if (guessedWord[i] == '_') {
            dashes += "_ ";  
        } else {
...
void playGame() {
    ...
    while (window.isOpen()) {
        ...
        if (isGameOver != 0) {
            ...
            window.display();
            sf::sleep(sf::seconds(2));  // Pause to display game-over screen
            return;
        }
...
void drawGameScreen() {
    ...
    drawCategoryText();  // Draw category text
    drawDashes();        // Draw the word with dashes
    highscore.displayScore(window, font);
    drawHintsLeft();     // Draw the number of hints left
...
public:
	...
	snake_sound()
	{
		if (!eating.loadFromFile("sfx/snake eating.wav"))
		{
			cout << "\nfailuire to load eating sfx";
		}
		if (!collision.loadFromFile("sfx/snake collision.wav"))
		{
...
public:
    ...
    void loadHighScore() 
    {
        ifstream file("SnakeHighScore.txt");
        ifstream scorefile("snakescorepergame.txt");
...
public:
   Snake() : speed(0), direction(1, 0) 
   {
    ...
    if (inFile) {
     ...
     {
        cout << "No saved game found. Initializing default state." << endl;
...
public:
    Screen() {
        ...
        if (!splashTexture.loadFromFile("THEGAMEBOYsplash.png"))
         {
...
public:
    ...
    void update_achievements(const player& plyr)
    {
        ...
        // Set achievements for the snake game
        ...
        if (plyr.player_snake_score >= 500) {
            player_achievements_snake[4] = "Serpent Supreme";
        }

        // Set achievements for wordle game
        if (plyr.player_wordle_score >= 10) {
            player_achievements_wordle[0] = "Letter Learner";
        }
        if (plyr.player_wordle_score >= 50) {
            player_achievements_wordle[1] = "Word Wielder";
        }
    ...
    // Returning copies so that original arrays are not modified elsewhere
   const string* get_snake_achievement(int index) {
    return &player_achievements_snake[index];
}

const string* get_wordle_achievement(int index) {
    return &player_achievements_wordle[index];
}

const string* get_hangman_achievement(int index) {
    return &player_achievements_hangman[index];
}
...
private:
    ...
    string snakeTopPlayer;
    string wordleTopPlayer;
    string hangmanTopPlayer;

public:
    Leaderboard()
        : playerCount(0), snakeHighScore(0), wordleHighScore(0), hangmanHighScore(0),
          snakeTopPlayer(""), wordleTopPlayer(""), hangmanTopPlayer("") {
        loadPlayersData();
        loadHighScores();
    }
...
void loadPlayersData() {
    ifstream inFile("players_data.txt");
    if (!inFile.is_open())
     {
        cout << "Could not open players data file!" << endl;
        return;
    }
    ...
    while (inFile.getline(line, 256) && playerCount < MAX_PLAYERS) {
        char name[100];
        int snakeScore = 0, wordleScore = 0, hangmanScore = 0;
        int i = 0, j = 0;
        ...
        if (line[i] == ',') i++; // Move past the comma

        // Parse snakeScore
        while (line[i] >= '0' && line[i] <= '9') {
            snakeScore = snakeScore * 10 + (line[i] - '0');
            i++;
        }
        if (line[i] == ',') i++; // Move past the comma
        ...
        // Parse wordleScore
        while (line[i] >= '0' && line[i] <= '9') {
            wordleScore = wordleScore * 10 + (line[i] - '0');
            i++;
        }
        if (line[i] == ',') i++; // Move past the comma
        ...
        // Parse hangmanScore
        while (line[i] >= '0' && line[i] <= '9') {
            hangmanScore = hangmanScore * 10 + (line[i] - '0');
            i++;
        }

        // Create a player object and add to the leaderboard
        player newPlayer(name, snakeScore, wordleScore, hangmanScore);
        addPlayer(newPlayer);
    }
...
void loadHighScores() {
  ...
  //  ifstream hangmanFile("hangman_highscore.txt");
    ...
    // Check and load Snake highscore
    if (snakeFile.is_open()) {
        snakeFile >> snakeHighScore;
        if (snakeFile.fail()) {
            snakeHighScore = 0;  // If reading fails, set to 0
        }
        snakeFile.close();
    } else {
        cout << "Could not open SnakeHighScore.txt!" << endl;
        snakeHighScore = 0;  // Initialize to 0 if file doesn't exist
    }

    // Check and load Wordle highscore
    if (wordleFile.is_open()) {
        wordleFile >> wordleHighScore;
        if (wordleFile.fail()) {
            wordleHighScore = 0;  // If reading fails, set to 0
        }
        wordleFile.close();
    } else {
        cout << "Could not open wordle_highscore.txt!" << endl;
        wordleHighScore = 0;  // Initialize to 0 if file doesn't exist
    }

   /** // Check and load Hangman highscore
    if (hangmanFile.is_open()) {
        hangmanFile >> hangmanHighScore;
        if (hangmanFile.fail()) {
            hangmanHighScore = 0;  // If reading fails, set to 0
        }
        hangmanFile.close();
    } else {
        cout << "Could not open hangman_highscore.txt!" << endl;
        hangmanHighScore = 0;  // Initialize to 0 if file doesn't exist
    }*/
}

// Function to save high scores to text files
void saveHighScores() {
    ofstream snakeFile("SnakeHighScore.txt");
    ofstream wordleFile("wordle_highscore.txt");
  //  ofstream hangmanFile("hangman_highscore.txt");

    // Save Snake highscore
    if (snakeFile.is_open()) {
        snakeFile << snakeHighScore;
        snakeFile.close();
    } else {
        cout << "Cannot save Snake highscore!" << endl;
    }

    // Save Wordle highscore
    if (wordleFile.is_open()) {
        wordleFile << wordleHighScore;
        wordleFile.close();
    } else {
        cout << "Cannot save Wordle highscore!" << endl;
    }

    /*// Save Hangman highscore
    if (hangmanFile.is_open())
     {
        hangmanFile << hangmanHighScore;
        hangmanFile.close();
     ...
     {
        cout << "Cannot save Hangman highscore!" << endl;
    }*/
}
...
void addPlayer(const player& plyr)
 {
    if (playerCount < MAX_PLAYERS) 
    {
        ...
        saveHighScores(); // Ensure high scores are saved after each addition
    } else {
...
void updateHighScores(const player& plyr) {
    if (plyr.player_snake_score > snakeHighScore) {
        ...
        snakeTopPlayer = plyr.name;
        achievements[playerCount - 1].player_achievements_snake[4] = "Snake Champion"; // Award top player
    }
    if (plyr.player_wordle_score > wordleHighScore) {
        wordleHighScore = plyr.player_wordle_score;
        wordleTopPlayer = plyr.name;
        achievements[playerCount - 1].player_achievements_wordle[4] = "Wordle Champion"; // Award top player
    }
    if (plyr.player_hangman_score > hangmanHighScore) {
        hangmanHighScore = plyr.player_hangman_score;
        hangmanTopPlayer = plyr.name;
        achievements[playerCount - 1].player_achievements_hangman[4] = "Hangman Champion"; // Award top player
    }
}
    ...
    for (int i = 0; i < playerCount - 1; ++i) {
        for (int j = 0; j < playerCount - i - 1; ++j) {
            if (players[j].player_snake_score < players[j + 1].player_snake_score) {
 ...
 void displayLeaderboard()
  ...
  {
     loadHighScores();
     updateHighScores(players[playerCount - 1]);
    ...
    if (! bgTexture.loadFromFile("image1.jpeg"))
     {
    ...
    while (window.isOpen()) 
    {
        ...
        // High Scores Display
        sf::Text highScores("High Scores:\n" "Snake: " + to_string(snakeHighScore) + "\n" +"Wordle: " + to_string(wordleHighScore) + "\n" +"Hangman: " + to_string(hangmanHighScore),
                            font, 20);
        highScores.setFillColor(sf::Color::Black);
        highScores.setPosition(50, 80);
        window.draw(highScores);



        // Display Champions and Achievements (only once)
        sf::Text championsTitle("Champions and Achievements", font, 30);
        championsTitle.setFillColor(sf::Color::Black);
        championsTitle.setPosition(50, 170);
        window.draw(championsTitle);



        // Snake Champion
        sf::Text snakeChampion("Snake Champion: " + snakeTopPlayer + "\nAchievement: " + *achievements[playerCount - 1].get_snake_achievement(4), font, 20);
        snakeChampion.setFillColor(sf::Color::Black);
        snakeChampion.setPosition(50, 200);
        window.draw(snakeChampion);



        // Wordle Champion
        sf::Text wordleChampion("Wordle Champion: " + wordleTopPlayer + "\nAchievement: " + *achievements[playerCount - 1].get_wordle_achievement(4), font, 20);
        wordleChampion.setFillColor(sf::Color::Black);
        wordleChampion.setPosition(50, 250);
        window.draw(wordleChampion);



        // Hangman Champion
        sf::Text hangmanChampion("Hangman Champion: " + hangmanTopPlayer + "\nAchievement: " + *achievements[playerCount - 1].get_hangman_achievement(4), font, 20);
        hangmanChampion.setFillColor(sf::Color::Black);
        hangmanChampion.setPosition(50, 300);
        window.draw(hangmanChampion);



        // Player Achievements Display
        sf::Text achievementsTitle("Player Achievements", font, 30);
        achievementsTitle.setFillColor(sf::Color::Black);
        achievementsTitle.setPosition(50, 400);
        window.draw(achievementsTitle);



        float yOffset = 450; // Adjust starting position for player list
        for (int i = 0; i < playerCount; ++i)
         {


            // Construct the player details string
            string playerDetailsStr =
                "Player: " + players[i].name + "\n" +
                "Snake Achievements: " + *achievements[i].get_snake_achievement(4) + "\n" +
                "Wordle Achievements: " + *achievements[i].get_wordle_achievement(4) + "\n" +
                "Hangman Achievements: " + *achievements[i].get_hangman_achievement(4);
            
            sf::Text playerDetails(playerDetailsStr, font, 18);
            playerDetails.setFillColor(sf::Color::Black);
            playerDetails.setPosition(50, yOffset);



            window.draw(playerDetails);

            yOffset += 150; // Adjust space between players to avoid overlap
        }
...
public:
    ...
    static void handleMenuInput(sf::Keyboard::Key key, int& i, int optionCount)
     {


        if (key == sf::Keyboard::Up && i > 0)
         {

           i--;
        } 
        
        else if (key == sf::Keyboard::Down && i < optionCount - 1) 
        {
            i++;
        }
...
// ................................................... WordleGame Class .............................................................
class startWordleGame : public Game
 {
public:
    void start() override 
    {
    ...
    RenderWindow window(VideoMode(1280, 720), "Keyboard Test");
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == Event::KeyPressed)
             {
               // virtualKeyboard.handle_keyboard_input(event.key.code);
                ...
                if (event.mouseButton.button == Mouse::Left)
                 {
                  ...
                  //  virtualKeyboard.handle_mouse_input(mousePos);
                }
            }
        }
    ...
    void displayGameOptions(Screen* screen)
     {
        ...
        switch (gameMenu.selectedoption())
         {
            ...
            case 2:
                wordleGame.start();
                break;
            case 3:                                   //to resume previous game
...
public:
    GameBoy(Screen* screenPtr): screen(screenPtr),choice(screenPtr, new string[4]{"Games", "LeaderBoard", "Players", "Exit"}, 4) {}
    ...
    void start() {
        ...
        {
            cout << "Error: Screen not initialized!" << endl;
        }
    }
